...
import dev.hypercore.world.FabricWorldStateApplier;
...
/**
 * Fabric-specific bridge wiring.
 *
 * <p>Opens a {@link HostBridge} when the process runs as a {@code FABRIC_HOST},
 * attaches the Fabric world delta applier, routes local world mutations into the
 * bridge, mirrors block-event cancellations, and announces player join/quit to
 * the other host. Returns {@code null} in standalone mode so existing Fabric
 * servers are completely unaffected.
 */
public final class FabricBridgeEndpoint implements AutoCloseable {
    private static final Logger LOGGER = LoggerFactory.getLogger(FabricBridgeEndpoint.class);
    ...
    private FabricBridgeEndpoint(HostBridge bridge, HyperCoreRuntime runtime, MinecraftServer server) {
    ...
    /**
     * Opens the bridge when this process runs as a Fabric host.
     *
     * @return the endpoint, or {@code null} when bridging is disabled
     */
    public static FabricBridgeEndpoint open(HyperCoreRuntime runtime, MinecraftServer server) {
        ...
        FabricBridgeEndpoint endpoint = new FabricBridgeEndpoint(bridge, runtime, server);
        endpoint.wire();
        bridge.start();
        LOGGER.info(
            "Fabric bridge endpoint opened: {} connecting to {}:{} (tick {} ms)",
    ...
    private void wire() {
        bridge.setApplier(new FabricWorldStateApplier(server, bridge.eventProxy()));
        bridge.setOnCommandsMirrored(() -> dev.hypercore.plugin.FabricPluginCommandBridge.register(
