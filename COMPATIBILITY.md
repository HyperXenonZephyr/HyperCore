...
3. ~~Scheduler, command, permission, and event conformance tests against selected reference plugins.~~ Done — scheduler, command, lifecycle, tab completion, permission children, and cross-plugin lookup paths are unit-tested with an `ExampleBukkitPlugin` fixture; event and permission conformance are covered by the GameTest fixture and unit tests.
4. ~~A per-plugin test matrix covering startup, reload rejection, shutdown cleanup, and Forge/Fabric mod coexistence.~~ Done — startup, command dispatch, scheduling, shutdown cleanup, cross-plugin lookup, world block read/write, `BlockState` update, entity spawn, entity teleport, custom name, player display name/game mode/teleport, block-entity inventory, Bukkit event bridge, `plugin.yml` permission registration, `WorldCreator`, and region-parallel execution are covered; reload rejection is implemented (`Server.reload`, `PluginManager.enablePlugin` throw `UnsupportedOperationException`; `PluginManager.disablePlugin` disables and cleans up) and verified by unit tests.
