...
4. A per-plugin test matrix covering startup, reload rejection, shutdown cleanup, and Forge/Fabric mod coexistence. Partial — startup, command dispatch, scheduling, shutdown cleanup, cross-plugin lookup, world block read/write, `BlockState` update, entity spawn, entity teleport, block-entity inventory, Bukkit event bridge, and region-parallel execution are covered; reload rejection and Forge/Fabric mod coexistence are pending.
5. Dedicated-server GameTests and end-to-end behavior checks without unsupported thread access. Done — both Forge and Fabric `runGameTestServer` load a dedicated test Bukkit plugin from `run/plugins` and verify commands, block/entity/inventory APIs, event bridging, and multi-owner region ticks in a real server.
6. World, block, block-entity, inventory, and entity mutation API conformance. Partial — `World.getBlockAt`, `Block.getType`/`setType`, `BlockState.update` (including `getInventory` for chests), `World.spawnEntity`, `World.getEntities`, `Entity.teleport`, `Inventory` set/read, and `ItemStack` amount/durability are implemented and verified in Forge/Fabric GameTests; player inventory, item meta beyond amount/durability, and advanced entity APIs are not supported.
7. Bidirectional Bukkit event bridge for world/block/entity events. Done — `BlockPlaceEvent`, `BlockBreakEvent`, `EntitySpawnEvent`, `PlayerMoveEvent`, and `PlayerJoinEvent` are bridged and cancellation is propagated back to abort HyperCore mutations; verified in Forge/Fabric GameTests.
